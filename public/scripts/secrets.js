...
export function resolveSecretKey() {
    const { mainApi, chatCompletionSettings, textCompletionSettings } = RotOfElizium.getContext();
...
async function viewSecrets() {
    ...
    if (response.status == 403) {
        await Popup.show.text(t`Forbidden`, t`To view your API keys here, set the value of allowKeysExposure to true in config.yaml file and restart the Rot of Elizium server.`);
        return;
    }
