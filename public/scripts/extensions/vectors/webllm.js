export class WebLlmVectorProvider {
     ...
     */
    #checkWebLlm() {
        if (!Object.hasOwn(RotOfElizium, 'llm')) {
            throw new Error('WebLLM is not available', { cause: 'webllm-not-available' });
        }

        if (typeof RotOfElizium.llm.generateEmbedding !== 'function') {
    ...
    #initEngine(modelId) {
        this.#checkWebLlm();
        if (!this.#engine) {
            this.#engine = RotOfElizium.llm.getEngine();
        }
    ...
    getModels() {
        this.#checkWebLlm();
        return RotOfElizium.llm.getEmbeddingModels();
    }
