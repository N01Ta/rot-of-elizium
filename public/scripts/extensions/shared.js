    ...
    if (!('llm' in RotOfElizium)) {
        ...
        if (!sessionStorage.getItem(warningKey)) {
            toastr.error('WebLLM extension is not installed. Click here to install it.', 'WebLLM', {
                ...
                extendedTimeOut: 0,
                preventDuplicates: true,
                onclick: () => openThirdPartyExtensionMenu('https://github.com/RotOfElizium/Extension-WebLLM'),
            });
            sessionStorage.setItem(warningKey, '1');
        }
        return false;
...
export class ConnectionManagerRequestService {
    ...
    static async sendRequest(profileId, prompt, maxTokens, custom = this.defaultSendRequestParams, overridePayload = {}) {
        ...
        const context = RotOfElizium.getContext();
    ...
    */
    ...
    static getProfile(profileId) {
        const profile = RotOfElizium.getContext().extensionSettings.connectionManager.profiles.find((p) => p.id === profileId);
    ...
    static getProfileIcon(profileId) {
        if ((RotOfElizium.getContext()).extensionSettings.disabledExtensions.includes('connection-manager')) {
            return null;
        }

        const id = profileId ?? (RotOfElizium.getContext()).extensionSettings.connectionManager.selectedProfile;
        if (!id) return null;
