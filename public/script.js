...
export const systemUserName = 'Rot of Elizium System';
...
export let displayVersion = 'Rot of Elizium';
...
export let CLIENT_VERSION = 'RotOfElizium:UNKNOWN'; // For Horde header
...
async function getClientVersion() {
    try {
        ...
        displayVersion = `Rot of Elizium ${data.pkgVersion}`;
...
async function firstLoadInit() {
    ...
    splashLogo.alt = 'Rot of Elizium';
    splashLogo.className = 'splash-logo';
    splashLogo.ariaLabel = t`Rot of Elizium Logo`;
...
export function getOverswipeBehavior(messageId, message = undefined) {
    ...
    //The first message in a priistine chat will loop. It's chevrons will always be visible https://github.com/RotOfElizium/RotOfElizium/pull/4712#issuecomment-3557893373
...
export function refreshSwipeButtons(updateCounters = false, fade = true) {
    ...
    messageElements.each((index, div) => {
        ...
        //Chevrons should not fade-in during printMessages. //https://github.com/RotOfElizium/RotOfElizium/pull/4712#issuecomment-3539315919
        ...
        if (isMessageSwipeable(messageId, message)) {
            ...
            // Chevrons should always be shown on pristine greetings: https://github.com/RotOfElizium/RotOfElizium/pull/4712#issuecomment-3557893373
...
export async function swipe(event, direction, { source, repeated, message = chat[chat.length - 1], forceMesId, forceSwipeId, forceDuration } = {}) {
    ...
    function expandNewMessage(thisMesDiv) {
        ...
        thisMesDiv.animate({ height: new_height + 'px' }, {
            duration: 0, //used to be 100 //Disabled on Cohee's request. https://github.com/RotOfElizium/RotOfElizium/pull/4610/files#r2408731744
            queue: false,
    ...
    $('#chat_import_file').on('change', async function (e) {
        ...
        for (const file of targetElement.files) {
            ...
            if (selected_group && format === 'json') {
                toastr.warning(t`Only Rot of Elizium's own format is supported for group chat imports. Sorry!`);
                continue;
            }
